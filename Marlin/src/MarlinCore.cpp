...
#if ENABLED(CUSTOM_USER_Botones)
  #if HAS_BOTON_USUARIO_1
  // #define MAIN_MENU_ITEM_2_GCODE "M140 S" STRINGIFY(PREHEAT_1_TEMP_BED) "\nM104 S" STRINGIFY(PREHEAT_1_TEMP_HOTEND)
    #define Mensaje1 HACEMOS AUTOHOME//= "HACEMOS AUTOHOME"
    #define Mensaje2 EXTRUIR

    #define Mensaje3 TEMP_SENSOR_1


    static int BotonUsuario1BtnCounter = 0;
    static uint8_t BotonUsuario1BtnFlag = 0;
    const int BOTON_USUARIO_1_DELAY = 750;
    if(!READ(BOTON_USUARIO_1_PIN))
      BotonUsuario1BtnCounter++;
    else
      BotonUsuario1BtnCounter = 0;
    if (BotonUsuario1BtnCounter == BOTON_USUARIO_1_DELAY)
    {
      switch (BotonUsuario1BtnFlag)
      {
      case 0:
        //enqueue_and_echo_commands_P(PSTR("M140 S100"));
        //queue.inject(F("M140 S100"));
 //       queue.inject(F("M117 " STRINGIFY(Mensaje1) "\nM104 S200 \nM105 \nM109 S200 \nM300 S400 P200 \nG4 P500 \nG1 E100 F1000 \nM300 S400 P200 \nG4 P500 "));
        queue.inject(F("G1 Z30 F1000 \nM104 S200 \nM105 \nM109 S200 \nM300 S400 P200 \nG4 P500 \nG1 E100 F1000 \nM300 S400 P200 \nG4 P500 \nM82"));
        BotonUsuario1BtnFlag++;  //++;
        break;
        //m140 cama m104 hotend
      case 1:
        //enqueue_and_echo_commands_P(PSTR("M140 S100"));
        queue.inject(F("M104 S0"));
        //queue.inject(F("M117 " STRINGIFY(Mensaje2) "\nM104 S200 \nM109 S200 \nG1 E100 F1000 "));//"\nG1 X0 Y0 Z20 F1000 "));
        BotonUsuario1BtnFlag++;
        break;
        //STRINGIFY(PREHEAT_2_TEMP_HOTEND) 
       
//  PREHEAT_3_TEMP_HOTEND Pet

      case 2:
        //enqueue_and_echo_commands_P(PSTR("M140 S0"));
        queue.inject(F("M104 S" STRINGIFY(PREHEAT_2_TEMP_HOTEND)));
        //queue.inject(F("G1 Y30 F1000"));
        BotonUsuario1BtnFlag++;   //OJO EL ULLTIMO CASE ES CERO customGreenBtnFlag = 0;
        break;
      case 3:
        //enqueue_and_echo_commands_P(PSTR("M140 S0"));
        queue.inject(F("M104 S0"));
      ...
      case 4:
        //enqueue_and_echo_commands_P(PSTR("M140 S0"));
        //queue.inject(F("M140 S0"));
        queue.inject(F("G1 Y30 F1000"));
        BotonUsuario1BtnFlag = 0;   //OJO EL ULLTIMO CASE ES CERO customGreenBtnFlag = 0;
        break;
...
//----------------------------------------------------------
/*
  //#define BOTON_USUARIO_1_PIN 28
  //#define BOTON_USUARIO_1_PIN -1
  const int BOTON_USUARIO_1_DELAY = 750;
  static int customGreenBtnCounter = 0;
  #if PIN_EXISTS(BOTON_USUARIO_1) //_PIN)
 //static int customGreenBtnCounter = 0;
  //const int BOTON_USUARIO_1_DELAY = 750;
    //queue.inject(F("G28")); 
    if(!READ(BOTON_USUARIO_1_PIN))
    {
      ...
      queue.inject(F("M106 S200")); 
    }
    else
    {
      customGreenBtnCounter = 0;
      queue.inject(F("M104 S100"));
      if (customGreenBtnCounter == BOTON_USUARIO_1_DELAY)
      {
        //enqueue_and_echo_commands_P(PSTR("G28"));
        //enqueue_and_echo_commands_P(PSTR("M18"));
        queue.inject(F("M104 S200"));     //comienza a calentar solo
      }
      //endif
    }
...
//**************************************************************
//#if HAS_BOTON_USUARIO_1
//    SET_INPUT_PULLUP(BOTON_USUARIO_1);
//#endif
